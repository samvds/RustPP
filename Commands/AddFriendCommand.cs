...
namespace RustPP.Commands
{
    ...
    internal class AddFriendCommand : ChatCommand
    {
        ...
        public void AddFriend(PList.Player friend, Fougerite.Player friending)
        {
            ...
            {
                friending.MessageFrom(Core.Name, yellow + "☢ " + red + "You can't add yourself as a friend.");
                return;
            }
            ...
            {
                friending.MessageFrom(Core.Name, string.Format(yellow + "☢ " + red + "You are already friends with: " + yellow + "{0}" + red + ".", friend.DisplayName));
                return;
            }
            list.AddFriend(SecurityElement.Escape(friend.DisplayName), friend.UserID);
            command.GetFriendsLists()[friending.UID] = list;
            friending.MessageFrom(Core.Name, string.Format(yellow + "☢ " + green + "You have added: " + yellow + "{0}" + green + " to your friends list.", friend.DisplayName));
            Fougerite.Player ffriend = Fougerite.Server.GetServer().FindPlayer(friend.UserID.ToString());
            if (ffriend != null)
                ffriend.MessageFrom(Core.Name, string.Format(yellow + "☢ " + green + "Player: " + yellow + " {0}" + green + " has added you to their friends list.", friending.Name));
        }
    }
}